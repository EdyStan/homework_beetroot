...
class Item:
    ...
    def add_item(self, conn):
        with conn:
            c = conn.cursor()
            c.execute("SELECT EXISTS(SELECT name, type FROM items WHERE name= :name and type= :type)",
            ...
            if record[0] == 1:
                ...
                c.execute("""UPDATE items
                          ...
                          {'name': self.name, 'type': self.item_type, 'quantity': self.quantity})
            else:
                c.execute("INSERT INTO items (name, type, price, quantity) VALUES (:name, :type, :price, :quantity)",
                          {'name': self.name, 'type': self.item_type, 'price': self.price, 'quantity': self.quantity})

    def del_item(self, conn):
        with conn:
            c = conn.cursor()
            c.execute("DELETE FROM items WHERE name= :name AND type= :type",
                      {'name': self.name, 'type': self.item_type})
